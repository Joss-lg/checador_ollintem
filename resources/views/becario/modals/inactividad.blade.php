{{--
    Modal de inactividad — se abre desde becario-dashboard.js cuando el
    usuario lleva INACTIVITY_WARN_MINUTES sin interactuar con la página.
    Si no responde en INACTIVITY_CLOSE_MINUTES más, el sistema registra
    la salida automáticamente vía POST /salida/inactividad.
--}}
    ...
    <div class="modal-dialog relative w-full max-w-md mx-4 bg-white dark:bg-[#1b1e24] text-gray-900 dark:text-white border border-[#EAE4D8] dark:border-white/10 rounded-2xl shadow-xl dark:shadow-2xl overflow-hidden transform scale-95 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]">
        ...
        <div class="flex items-center justify-between px-6 pt-6 pb-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-xs font-bold tracking-widest text-gray-500 dark:text-gray-400 uppercase mb-1">Aviso de inactividad</p>
                    <h5 class="text-lg font-bold m-0" id="modalInactividadLabel">
                        ¿Sigues trabajando?
                    </h5>
                </div>
            </div>
        ...
        <div class="px-6 py-5 space-y-3">
            <p class="text-sm leading-relaxed text-gray-600 dark:text-gray-300 m-0">
                No hemos detectado actividad en tu sesión desde hace un momento.
                Si no confirmas que sigues trabajando, tu salida se registrará automáticamente en:
            </p>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 px-6 py-5 bg-slate-50 dark:bg-white/5 transition-colors">
            ...
            {{-- Cerrar sesión ahora (el usuario confirma que ya terminó) --}}
            <form id="formSalidaInactividad" method="POST" action="{{ route('salida.inactividad') }}">
                @csrf
                <button type="submit"
                        class="w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-white border border-gray-300 dark:border-white/20 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 transition-colors focus:ring-2 focus:ring-gray-200 dark:focus:ring-white/20 focus:outline-none">
                    Registrar mi salida ahora
                </button>
            </form>

            {{-- Seguir trabajando (reinicia el timer) --}}
            <button type="button"
                    id="btnSigoTrabajando"
                    class="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-amber-500 rounded-lg shadow-[0_4px_14px_0_rgba(245,158,11,0.35)] hover:bg-amber-400 hover:shadow-[0_6px_20px_rgba(245,158,11,0.25)] hover:-translate-y-0.5 transition-all focus:ring-2 focus:ring-amber-400 focus:outline-none">
